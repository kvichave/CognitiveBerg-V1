...
export default function MicrophoneComponent() {
  ...
  useEffect(() => {
    ...
    websocketRef.current.on("audio_urls", (files) => {
      ...
      console.log("WebSocket audio_urls event received", files);
      // console.log(
      //   "WebSocket audio_urls event received",
      //   files.interviewers[0]
      // );

      console.log("inter::  ", files);

      const inter = Array.isArray(files.investors)
        ? files.investors
        : [files.investors];

      console.log("Type of files.investors:", typeof files.investors);
      ...
      setSpeakers((prevSpeakers) => {
        ...
        return prevSpeakers.map((speaker) => {
          const updateInvestor = inte.find((i) => i.id === speaker.id);
          return updateInvestor
            ? { ...speaker, message: updateInvestor.message }
            : speaker;
        });
      });
    ...
    <div className="bg-gray-900">
      <div className="relative z-50">
        ...
        <div className="flex flex-row items-center justify-end h-screen w-full">
          <div className=" mt-24 ml-4 mr-2 grid grid-rows-2 grid-flow-col gap-1 ">
            {speakers.length > 0 ? (
              speakers.map((user, index) => (
                <div
                  key={user.id}
                  className={`ml-2 mb-20  animate-slide-in-up opacity-100 delay-${
                    index * 100
                  }`}
                >
                  {user.isSpeaking ? (
 ...
 rounded-2xl h-[70vh]  flex items-center justify-center"
            ...
            <div className="flex z-50 items-center justify-center w-full relative">
              <button
                className={`${
                  stream ? "bg-red-500 text-white" : "bg-white text-black"
                }  hover:bg-gray-100 hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg text-sm px-5 py-2.5 text-center`}
                onClick={() => {
                  if (stream) {
                    stopScreenCapture();
                  } else {
