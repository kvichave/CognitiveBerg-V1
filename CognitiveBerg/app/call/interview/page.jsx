...
export default function MicrophoneComponent() {
    ...
    websocketRef.current.on("audio_urls", (files) => {
      ...
      setSpeakers((prevSpeakers) => {
        ...
        return prevSpeakers.map((speaker) => {
          const updatedInterviewer = inte.find((i) => i.id === speaker.id);
          return updatedInterviewer
            ? { ...speaker, message: updatedInterviewer.message }
            : speaker;
        });
      });
  ...
  useEffect(() => {
    // Play audio when audiourls are set
    const playAudio = () => {
  ...
  return (
    <BackgroundGradientAnimation>
      ...
      <button
        onClick={hideunhidecode}
        className="p-2 rounded-md bg-blue-500 hover:bg-blue-600 text-white transition-all flex items-center justify-center absolute top-4 right-4 z-[100]"
      >
        <svg
